...
to the method and the model rather than to our harness. Three interventions
...
remaining two are changes to the implementation, and are the reason this
...
Both changes are confined to the harness that surrounds the roles: one to how
it inspects the repository, one to how it constructs a request. The three role
prompts, the tools available to each role, the structure of the session loop,
the order in which the roles run, and the information passed between them are
unmodified. No step was added to any role's instructions, and no role was given
a capability it did not previously have.
...
unmodified implementation on this class of task would in effect be a comparison
against a two-role method. Without the second, two runs of the same written
configuration are not guaranteed to have used the same sampling or the same
amount of reasoning, which is the property a controlled comparison depends on.
